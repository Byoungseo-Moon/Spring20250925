...
import java.util.Random;
	...
	public class SmsController {
	     ...
	     * 단일 메시지 발송 예제
	     */
	    @PostMapping("/send-one") // member-join.jsp에서 fnSms function ajax 실행
	    
//	    public SingleMessageSentResponse sendOne() { 한개 보낼 때
	    public HashMap<String, Object> sendOne() {	
	        Message message = new Message();
	        // 발신번호 및 수신번호는 반드시 01012345678 형태로 입력되어야 합니다.
	        String ranStr = randomNumber();
	        message.setFrom("01046548947"); // api key를 가진사람의 번호
	        message.setTo("01090802306");
	        message.setText("[본인 인증] 인증번호 " + ranStr + "를 화면에 입력해 주세요.");
